...
**`undo_last_edit`** —— 撤销某个文件的**上一次编辑**，内容与编码一并还原：
...
- **编码一起还原**。如果那次编辑把 GBK 文件转成了 UTF-8（见 `normalizeToUtf8`），撤销会把文件**变回 GBK**，而不只是内容回去。
- **文件被改过就拒绝**。撤销前会核对文件是否仍等于那次编辑写入的内容；如果之后有人改过（你、其他工具、其他会话），撤销会**拒绝并说明**，绝不覆盖那些改动。
...
> **⚠️ 冲突提示**：**任何**在同一个作用域层注册 `read` / `write` / `edit` / `insert` / `str_replace_editor` / `undo_last_edit` 中任一名字的插件都与本插件互斥——同一层重复注册同名工具会直接报错。
>
...
- **与任何注册同名工具的插件互斥**：本插件在作用域层注册 `read` / `write` / `edit` / `insert` / `str_replace_editor` / `undo_last_edit`，而同一层重复注册同名工具会直接报错。判定依据是这些名字在**该 agent 自己的层**上是否已被占用，与对方是谁无关——发现冲突即拒绝安装并指出被占用的工具名（见上方安装说明）。宿主／preset 层的内建工具不属于冲突。
